...
```mermaid
graph TD
    UI[Creator Suite PyQt6 UI] --> GameTools[Interactive Narrative Toolkit]
    ...
    NPCForge --> UnrealExport[Export Layer Layer]
...
6. **Export Layer Layer (`modules/export_layer/`)**: A serialization pipeline that converts nested dictionaries into flat CSVs for DataTables.
